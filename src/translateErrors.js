...
const daysDataPath = join(__dirname, `days/errorLogs/${year}-${month}_errorLog.json`);
...
(async () => {
  ...
  const outputFile = `${outputDir}/spanish/${year}-${month}_fixes.json`;
  ...
  if (errorLog.length > 0) {
    console.log(`Errors encountered. Check error log for details.`);
  }
})();